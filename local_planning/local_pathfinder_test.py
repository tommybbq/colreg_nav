...
class LocalPathfinder:
    def __init__(self, sailor: sailor.Sailor, waypoints: list) -> None:
        ...
        self.global_offtrack_flag: bool = True
    ...
    def set_waypoints(self, waypoints: list):
        """
        If passed new waypoints updates the relevant information
        """
    ...
    def update_navigation_parameters(self):
        """
        Update navgiation parameters that depend on the current waypoint
        """
        self._update_pos()
        if len(self.waypoints) > 1:
            self._update_azimuth_values(self, self.waypoints[0], self.waypoints[1])
        self._update_errors()


    def _update_pos(self):
        #Current boat latitude and longitude
        self.current_lat, self.current_lon = self.sailor.boat.coords
    ...
    def _run_flag_check(self):
        ...
        if abs(self.cross_track_error) > global_offtrack_check:
            self.global_offtrack_flag = False
            return False
    ...
    def run(self) -> bool:
        ...
        while main_flag:
            ...
            if not self.cross_track_flag or not self.next_waypoint_flag or self.time_flag:
                self.send_flag = True
        ...
        if not main_flag:
            if not self.global_offtrack_flag or not self.stop_condition:
                return False
            else:
