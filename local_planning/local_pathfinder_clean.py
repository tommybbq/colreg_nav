...
from helper_functions import magnitude, haversine, find_bearing
import random
...
class LocalPathfinder:
    def __init__(self, sailor: sailor.Sailor, waypoints: list) -> None:
        ...
        self.sailor = sailor
        self.waypoints = waypoints                             #Allow for update waypoints from global
        self.waypoint_index = 0

        self.current_lat = self.sailor.boat.gps.lat                                #Boat's current latitude 
        self.current_lon = self.sailor.boat.gps.lon
        self.current_location = [self.current_lat, self.current_lon]                              #Boat's current longitude
    ...
    def _update_azimuth_values(self):
        self.azimuth_angle = radians(find_bearing(self.current_waypoint, self.next_waypoint))
        rotation_matrix = np.array([
            [cos(self.azimuth_angle), -sin(self.azimuth_angle)],
            [sin(self.azimuth_angle), cos(self.azimuth_angle)]
        ])
        self.rotation_matrix_trans = np.transpose(rotation_matrix)
        self._update_errors()
    ...
    def _update_errors(self):
        ...
        position_error_vector = np.dot(self.rotation_matrix_trans, position_difference_vector)
    ...
    def _adaptive_line_of_sight(self):
        ...
        #Update errors
        self._update_errors()
        print(f"Cross track error: {self.cross_track_error}")
        ...
        print(f"Desired heading: {self.desired_heading_angle}")
    ...
    def run(self):  
        if self.sailor.boat.gps.lat == 0 or self.sailor.boat.gps.lon == 0:
            return None
        print(f"Current lat,lon: {self.sailor.boat.gps.lat},{self.sailor.boat.gps.lon}")

        global_offtrack_check = 100
        cross_track_check = 25 # change as necessary
        waypoint_radius = 10
        max_heading_update_time = 100 
        ...
        if self.distance_to_next_waypoint < waypoint_radius:
            ...
            #If not the final waypoint
                ...
                print(f"Updating waypoint, new waypoint index: {self.waypoint_index}")
                self._update_waypoints()
                self.along_track_error: float = 0.0                         #Along track error
                self.cross_track_error: float = 0.0                         #Cross track error   
...
if __name__ == "__main__":
    ...
    #simple_waypoints = [[29.480423, -94.939422],[29.480423, -94.939422],[29.481166907335947, -94.93880206600495],[29.481594794682366, -94.93874222718301]]
